...
**Note:** loss was never the right metric here — the paper evaluates on FAD and pitch distance. Both `SampleLogger` and `Run_timbre_transfer.py` have since been fixed to pass `midi=` (see Bug 12 and the inference section); before that, no conditioned audio had ever been generated by anything.
...
**Bug 12 — `SampleLogger` never ran, and would have crashed if it had.**
Two separate faults in the callback that is supposed to write audio samples to the logger during validation:
- `self.log_next` was set to `False` in `__init__` and **never set to `True` anywhere**, so `log_sample()` was unreachable. Every run registered the callback and it silently did nothing — no run before 2026-09-07 produced any audio samples at all.
- `log_sample()` raised `ValueError("TensorBoardLogger not found...")` for any non-TensorBoard logger. Since `loggers: wandb` is the active default, simply setting `log_next = True` would have killed the run at the first validation.
