...
class UNet1d(nn.Module):
    def __init__(
        ...
        in_channels: int = 128,
        channels: int = 128,
        multipliers: Sequence[int] = [1, 2, 4, 8, 8, 8],
        factors: Sequence[int] = [4, 4, 4, 2, 2],
        num_blocks: Sequence[int] = [2, 2, 2, 2, 2],
        attentions: Sequence[int] = [0, 1, 1, 1, 1, 1],
        ...
        **kwargs,
        ...
        kwargs["cond"] = cond
        kwargs["diffusion_latent"] = diffusion_latent
        kwargs['attention_heads'] = attention_heads
        kwargs['attention_features'] = attention_features
        kwargs['attention_multiplier'] = attention_multiplier
        ...
        #if self.latent == True: #TODO set it to False
...
class UNetConditional1d(UNet1d):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if kwargs['embedding_mask_proba'] > 0.0:
...
class EnCodec(nn.Module):
    def __init__(self, model: str = "facebook/encodec_24khz "):
        super().__init__()
        #from transformers import EncodecModel
        from encodec import EncodecModel
        #self.encodec_model = EncodecModel.from_pretrained(model)
        self.encodec_model = EncodecModel.encodec_model_24khz()

    @torch.no_grad()
    def encode_latent(self, x: Tensor) -> Tensor:
        encoder = self.encodec_model.encoder()
        z = encoder(x)
        return z

    @torch.no_grad()
    def decode_latent(self, z: Tensor) -> Tensor:
        decoder = self.encodec_model.decoder()
        x = decoder(z)
        return x
